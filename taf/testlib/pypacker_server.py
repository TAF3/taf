...
class XMLPypackerServer(xmlrpc.XMLRPC):
    ...
    def __register_methods(self):
        ...
        for attr in pypacker_attrs:
            if isinstance(getattr(self.pypacker, attr), (str, int, list, dict)):
                setattr(self, "xmlrpc_" + attr, wrap_attibute(attr))
                self.class_logger.debug("Registered Pypacker TG attribute {0}", attr)
            elif callable(getattr(self.pypacker, attr)):
                setattr(self, "xmlrpc_{0}".format(attr), wrap_method(attr))
                self.class_logger.debug("Registered Pypacker TG method {0}", attr)
        ...
        try:
            ...
            for iface in data_orig:
                ...
                for packet in data_orig[iface]:
                    ...
                    _time = packet.time
                    data_str[iface].append(pickle.dumps([_time, packet.bin()]))
            return data_str
        except:
            exc_type, exc_value, exc_traceback = sys.exc_info()
            traceback_message = traceback.format_exception(exc_type, exc_value, exc_traceback)
    ...
    def xmlrpc_tgcmd(self, method, args=None, kwargs=None):
        ...
        str_kwargs = " ".join(["{0}={1}".format(k, kwargs[k]) for k in kwargs])
        self.class_logger.info("Command: {0}({1}, {2})", method, str_args, str_kwargs)
        try:
