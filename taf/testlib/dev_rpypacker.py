...
class RemotePypackerTG(PacketProcessor, TGHelperMixin, GenericEntry):
    ...
    def start(self, wait_on=True):
        ...
        if not self.opts.get_only:
            ...
            for _ in timeout_iter:
                if probe_port(self.ipaddr, self.ssh_port, self.class_logger):
                    break
                time.sleep(1)
        ...
        timeout_iter = itertools.takewhile(lambda x: x < end_time,
                                           iter(time.time, -1))
        for _ in timeout_iter:
            lport = self.ssh.exec_command("cat " + ppfile_path).stdout
    ...
    def stop(self):
        ...
        try:
            ...
            self.class_logger.error("Failed to copy logs from RPypacker server: {0}".
                                    format(err))
        # Stopping pypacker server
        try:
            self.xmlproxy.shutdown()
        except Exception as err:
            self.class_logger.debug("Shutdown ReturnCode: {0}".format(err))
    ...
    def get_logs(self):
        ...
        file_list = list(filter(bool, out.split("\n")))
        self.class_logger.debug("Files to copy: {0}".format(file_list))
    ...
    def stop_sniff(self, *args, **kwargs):
        ...
        """
        data_str = self.xmlproxy.stop_sniff(args, kwargs)
        data = defaultdict(list)
        for iface in data_str:
            for packet in data_str[iface]:
                _time, packet = pickle.loads(packet.data)
