        ...
        for layer in packet_definition:
            ...
            with suppress(KeyError):
                ...
                for tlv_dict in tlvlist_def:
                    ...
                    tlvlist.append(_pypacker_layer({tlv_type: OrderedDict(sorted(tlv_dict[tlv_type].items(), key=first))}))
                    with suppress(KeyError):
                        ...
                        tlvlist[-1].apppriotable = app_tables_list
                packet.lldp.tlvlist = tlvlist
            with suppress(KeyError):
