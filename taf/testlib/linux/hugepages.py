...
class HugePages(object):
    ...
    def mount(self, nr_hugepages, per_node=None, mnt_dir='/mnt/huge', hugepage_size=DEFAULT_HUGEPAGE_SIZE, **kwargs):
        ...
        if kwargs:
            cmd = "mount -t hugetlbfs nodev {mnt_dir} {cmd_args}".format(mnt_dir=mnt_dir,
                                                                         cmd_args=' '.join(' '.join(['--{}'.format(x), str(y)])
                                                                                           for x, y in kwargs.items()))
        else:
            cmd = "mount -t hugetlbfs nodev {mnt_dir}".format(mnt_dir=mnt_dir)
    ...
    def umount(self, mnt_dir='/mnt/huge', **kwargs):
        ...
        """
        if kwargs:
            cmd = 'umount {mnt_dir} {cmd_args}'.format(mnt_dir=mnt_dir,
                                                       cmd_args=' '.join(' '.join(['--{}'.format(x), str(y)])
                                                                         for x, y in kwargs.items()))
        else:
            cmd = 'umount {mnt_dir}'.format(mnt_dir=mnt_dir)
        self.cli_send_command(cmd)
